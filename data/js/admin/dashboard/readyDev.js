...
$('.sEditProduct').live('click', function(e){
    ...
    web.iReq({action: 'iGetContent', path: $(this).parent().parent().attr('path')}, function(c){
        $.each(c, function(key, value){
            $('#iProTitle'+value.language).val(value.title);
            $('#iProExcept'+value.language).val(value.except);
        });
        ...
        $.each(c, function(key, value){
            if(c[key].imgs!=null)
                $.each(c[key].imgs, function(k, v){
                    $('#fAddProduct .dTabsLanguage'+value.language+' .dPhoto').append('<p><img src="'+v+'" alt="wsup3"><a><img src="'+SKIN+'/admin/close.jpg" alt="wsup3"></a></p>');
                });
...
$('.sEditCategory').live('click', function(e){
    ...
    web.iReq({action: 'iGetContent', path: $(this).parent().parent().attr('path')}, function(c){
        ...
        $.each(c, function(key, value){
            $('#iCatTitle'+value.language).val(value.title);
            $('#iCatExcept'+value.language).val(value.except);
        });
        ...
        $.each(c, function(key, value){
            if(c[key].imgs!=null)
                $.each(c[key].imgs, function(k, v){
                    $('#fAddCategory .dTabsLanguage'+value.language+' .dPhoto').append('<p><img src="'+v+'" alt="wsup3"><a><img src="'+SKIN+'/admin/close.jpg" alt="wsup3"></a></p>');
                });
            if(c[key].params!=null)
                $.each(value.params, function(key, val){
                    num++;
...
$('.sEditPage').live('click', function(e){
    ...
    web.iReq({action: 'iGetContent', path: $(this).parent().parent().attr('path')}, function(c){
        $.each(c, function(key, value){
            $('#iPgTitle'+value.language).val(value.title);
            $('#iPgExcept'+value.language).val(value.except);
        });
        ...
        $.each(c, function(key, value){
            if(c[key].imgs!=null)
                $.each(c[key].imgs, function(k, v){
                    $('#fAddPage .dTabsLanguage'+value.language+' .dPhoto').append('<p><img src="'+v+'" alt="wsup3"><a><img src="'+SKIN+'/admin/close.jpg" alt="wsup3"></a></p>');
                });
            if(c[key].params!=null)
                $.each(value.params, function(key, val){
                    num++;
...
$('.sEditFeature').live('click', function(e){
    ...
    web.iReq({action: 'iGetContent', path: $(this).parent().parent().attr('path')}, function(c){
        $.each(c, function(key, value){
            $('#iFeaName'+value.language).val(value.title);
            $('#iFeaExcept'+value.language).val(value.except);
        });
        ...
        $.each(c, function(key, value){
            if(c[key].imgs!=null)
                $.each(c[key].imgs, function(k, v){
                    $('#fAddFeature .dTabsLanguage'+value.language+' .dPhoto').append('<p><img src="'+v+'" alt="wsup3"><a><img src="'+SKIN+'/admin/close.jpg" alt="wsup3"></a></p>');
                });
            if(c[key].params!=null)
                $.each(value.params, function(key, val){
                    num++;
...
$('.sEditMenu').live('click', function(e){
    ...
    web.iReq({action: 'iGetContent', path: $(this).parent().parent().attr('path')}, function(c){
        $.each(c, function(key, value){
            $('#iMnuName'+value.language).val(value.title);
            $('#iMnuExcept'+value.language).val(value.except);
        });
        ...
        $.each(c, function(key, value){
            if(c[key].imgs!=null)
                $.each(c[key].imgs, function(k, v){
                    $('#fAddMenu .dTabsLanguage'+value.language+' .dPhoto').append('<p><img src="'+v+'" alt="wsup3"><a><img src="'+SKIN+'/admin/close.jpg" alt="wsup3"></a></p>');
                });
            if(c[key].params!=null)
                $.each(value.params, function(key, val){
                    num++;
...
$('.sEditLang').live('click', function(e){
    ...
    web.iReq({action: 'iGetLanguage', id: $(this).prev().attr('lang')}, function(c){
        $.each(c, function(ky, val){
            $('#iName'+val.language).val(val.name);
        });
        $('#iId').val(c[0].id);
        ...
        $.each(c, function (key, value) {
            ...
            //        $('#fAddLanguage .dTabsLanguage'+value.language+' .dPhoto').append('<p><img src="'+v+'" alt="wsup3"><a><img src="'+SKIN+'/admin/close.jpg" alt="wsup3"></a></p>');
            //    });
            if(c[key].params!=null)
                $.each(value.params, function(key, val){
                    num++;
...
$('.dProduct').live('click', function(){
    ...
    TweenMax.to($(this).find('.dInfo'), 0.2, {paddingRight: '50px'});
    TweenMax.to($(this).find('.dOption'), 0.3, {right: 0, ease:Back.easeOut});
});
$('.dProductMore').live('click', function(){
    $(this).removeClass('dProductMore');
    TweenMax.to($(this).find('.dInfo'), 0.2, {paddingRight: '0px'});
    TweenMax.to($(this).find('.dOption'), 0.3, {right: '-50px'});
...
$('.dCategory .dProduct').live('click', function(){
    if($('#fCategories').width()-$(this).width()<$(this).position().left)
        TweenMax.to($('#fCategories'), 0.3, {scrollLeft: '+='+($(this).position().left-($('#fCategories').width()-$(this).width()))});
    else
    if($(this).position().left<0)
        TweenMax.to($('#fCategories'), 0.3, {scrollLeft: '+='+$(this).position().left});
});
$('.sSetDefault').live('click', function(){
    web.iReq({ action: 'iChangeLanguage', language: $(this).attr('lang') }, function (dt) {
        ...
        window.location.assign(BASEURL + '/admin/' + dt.language+'@'+dt._id);
    });
});
                                ...
                                success: function (dt) {
                                    if(dt.status == true)
                                    {
                                        $('.fImages').append('<div class="dNewItem">' +
                                            '<img uid="' + people._id + '" site="' + dt.link + '" _id="' + dt.mediaid + '" src="' + dt.link + '/multimedia/' + dt.mediaid + '" alt="wsup3" title="' + v.name + '"/>' +
                                            '<div class="dText">' +
...
$('.sBtnMore').live('click', function(){
    me = $(this);
    me.fadeOut(250);
    params = {action: 'iGetMores', type: me.attr('type'), from: me.prev().find('.dCategory').length, keys: $('.iSearch').val(), opt: undefined};
    switch(me.attr('type'))
    {
        case 'post':
        ...
        switch(me.attr('type'))
        {
            ...
            case 'image':
                if(dt.length>0)
                {
                    $.each(dt, function(k, v){
                        $('.fImages').append('<div class="dNewItem">' +
                            '<img uid="' + v.wid +'" site="'+v.site+'"_id="'+v.mediaid+'" src="' + v.site+'/multimedia/'+v.mediaid + '" alt="wsup3" title="'+v.name+'"/>' +
                            '<div class="dText">' +
                            '<span class="sGlass"></span>' +
                            '<span class="sTitle">'+v.name+'</span></div></div>');
                    });
            ...
            case 'friend':
                ...
                $.each(chats, function(k, v) {
                    ...
                    dt.forEach(function (obj, index) {
                        if (v._id == obj._id) {
                            console.log(v._id +', '+ obj._id);
                            dtTemp.splice(index, 1);
                            dtTemp.unshift(obj);
                            dtTemp[0].hasLetter = true;
                            return;
                        }
                    });
                });
...
$.each(languages, function(key, value){
    rules['name'+key] = {required: true, minlength: 2, maxlength: 1000};
    messages['name'+key] = {
        required: tran("Mời bạn nhập tên ngôn ngữ "+value+"|Enter "+value+" language name, please|Введите название "+value+" языка, пожалуйста|请输入"+value+"语言名称|"+value+"言語名を入力してください"),
        minlength: tran("Tên ngôn ngữ "+value+" quá ngắn, tối thiểu 2 ký tự|"+value+" language name too short, min 2 characters|"+value+" языка название слишком коротким, мин 2 символов|"+value+"语言名称太短，最少2个字符|"+value+"言語名が短すぎます（最小2文字）"),
        maxlength: tran("Tên ngôn ngữ "+value+" quá dài, tối đa 1000 ký tự|"+value+" language name too long, max 1000 characters|"+value+" Языка название слишком длинное, максимум 1000 символов|"+value+"语言名称太长，最多1000个字符|"+value+"言語名が長すぎます（最大1000文字）")
    };
});
$("#fAddUser").validate({
    rules: {
...
messages['id'] = {
    required: tran("Mời bạn nhập mã ngôn ngữ|Enter language code, please|Пожалуйста введите код языка,|请输入语言代码|言語コードを入力してください"),
                    ...
                    web.iReq({ action: 'iDeleteFile', id: $('.iViewImg').attr('_id'), site: $('.fImages .dItem:eq('+$('.dViewImg').attr('viewing')+') img').attr('site')}, function(dt){
                        if(dt.status == true)
                        {
                            ...
                            $('.iDel').fadeIn(200, function(){
                                if($('.aDelete').attr('deleting')!=0)
                                    $('.dItem > div:eq('+(parseInt($('.dViewImg').attr('viewing'))+1)+')').remove();
                                else
                                {
                                    $('.dItem > div:eq(0)').remove();
    ...
    if($(this).attr('time')==1) {
        ...
        switch (backmap.see().page) {
            ...
            case 'dBoardSell':
                ...
                break;
            default:
                $('.' + backmap.see().page+' .sBtnMore').prev().html('');
                $('.' + backmap.see().page+' .sBtnMore').trigger('click');
                setTimeout(function(){
                    $('.iSearch').attr('time', 1);
                }, 500);
