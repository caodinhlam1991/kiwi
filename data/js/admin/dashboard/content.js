var saveContent = function (type) {
    if ($('img.iCheck').css('display') == 'inline-block')
        ...
        if (typeof (temp) != 'undefined') {
            ...
            $.each(temp, function (key, value) {
                if (typeof (temp[key]) != 'undefined') {
                    item['_id' + value.flag] = temp[key]._id;
                    if (type != 'term')
                        item['postid' + value.flag] = temp[key].postid;
                }
            });
        }
        item.type = type;
        ...
        item.opath = $('.' + backmap.see().page + ' [name=path]').attr('placeholder');
        ...
        web.iReq({ action: 'iSaveContent', item: item }, function (dt) {
            switch (item.type) {
                case 'post':
                    animate.hidezoomout($('.dBoardAddProduct'), 288, function () {
                        $('#fProducts').html('');
                        $('.dAppProducts').trigger('click');
                    });
                    break;
                case 'term':
                    animate.hidezoomout($('.dBoardAddCategory'), 288, function () {
                        $('.fCats div').remove();
                        $('#fCategories').html('');
                    ...
                    break;
                case 'page':
                    animate.hidezoomout($('.dBoardAddPage'), 288, function () {
                        $('#fPages').html('');
                        $('.dAppPages').trigger('click');
                    });
                    break;
                case 'feature':
                    animate.hidezoomout($('.dBoardAddFeature'), 288, function () {
                        $('#fFeatures').html('');
                        $('.dAppFeatures').trigger('click');
                    });
                    break;
                case 'menu':
                    animate.hidezoomout($('.dBoardAddMenu'), 288, function () {
                        $('#fMenus').html('');
...
var saveUser = function () {
    if ($('img.iCheck').css('display') == 'inline-block')
        ...
        web.iReq({ action: 'iSaveUser', item: item }, function (data) {
            if (backmap.see().page == 'dBoardAddUser') {
                animate.hidezoomout($('.' + backmap.see().page), 288, function () {
                    $('#fUsers').html('');
...
var saveLanguage = function () {
    if ($('img.iCheck').css('display') == 'inline-block')
        ...
        item['namevi'] = $('#iNamevi').val();
        item['nameen'] = $('#iNameen').val();
        item['nameru'] = $('#iNameru').val();
        item['namech'] = $('#iNamech').val();
        item['nameja'] = $('#iNameja').val();
        ...
        web.iReq({ action: 'iSaveLanguage', item: item }, function (data) {
            animate.hidezoomout($('.dBoardAddLanguage'), 288, function () {
                $('#fLanguages').html('');
...
var getFeatures = function (idForm, value, key, val, num) {
    switch (val.tool) {
        case 'editor':
            ...
            $(idForm + ' .dTabsLanguage' + value.flag + ' .dFeatures').prepend(p + ((typeof (features[key]) == 'undefined') ? '' : (features[key]['da-gia-tri'] == 'show') ? '<a num="' + num + '" key="' + key + '" class="aValue">' + tran('Thêm nội dung|Add content||') + '</a>' : '') +
        ...
        case 'text':
            ...
            $(idForm + ' .dTabsLanguage' + value.flag + ' .dFeatures').prepend(p + ((typeof (features[key]) == 'undefined') ? '' : (features[key]['da-gia-tri'] == 'show') ? '<a num="' + num + '" key="' + key + '" class="aValue">' + tran('Thêm giá trị|Add value||') + '</a>' : '') +
                '</div></fieldset>').find('fieldset').first().fadeTo(300, 1);
            num2 = num;
            if (typeof (val.detail) == 'object') {
                if (typeof (val.parent) == 'object')
        ...
        case 'image':
            ...
            $(idForm + ' .dTabsLanguage' + value.flag + ' .dFeatures').prepend('<fieldset class="fFeature fFeature' + num + '" num="' + num + '" tool="' + val.tool + '" style="opacity: 0"><div>' +
                '<a class="aMinFeature" title="' + tran('Thu gọn|Minimize|||') + '"><i class="fa fa-minus"></i></a>' +
                '<a class="aDelFeature" title="' + tran('Hủy|Delete|||') + '"><img src="' + SKIN + '/admin/close.jpg" alt="wsup3"/></a>' +
                '<div class="dLine">' +
                '<label for="sFeatureName' + num + '">' + tran('Tên đặc điểm|Feature name|||') + ':&nbsp;</label>' +
